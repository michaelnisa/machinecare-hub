-- Workshop Business Settings (garage_workshop.md section 36). Scoped to
-- the fields that can be genuinely wired into real behavior rather than
-- sitting decorative:
--   - phone/address: shown on printed estimates/invoices and the public
--     job-status page.
--   - business_hours: shown on the public job-status page.
--   - invoice_footer_note: printed on invoices.
--   - accepted_payment_methods: filters the payment-method dropdown when
--     recording a payment.
--   - default_labour_rate_per_hour: quick-fill helper in the estimate
--     editor's labour cost field.
--   - default_message_channel: prefills the "Notify customer" channel.
-- Deliberately NOT building: a per-org "currency" toggle (the whole app's
-- money formatting is TZS-only by design, per CLAUDE.md's currency
-- convention — making that configurable is a much larger cross-module
-- change than a settings field), customizable job statuses (STATUS_FLOW is
-- relied on throughout the job lifecycle; making it editable risks
-- breaking status transitions elsewhere), and notification toggles beyond
-- default channel (there's no automated dispatch to toggle — see the
-- existing Reminders/MessageCard "log what you sent yourself" design).
...
ALTER TABLE public.organisations
  ...
  ADD COLUMN IF NOT EXISTS accepted_payment_methods text[] NOT NULL DEFAULT ARRAY['cash', 'mobile_money', 'bank', 'other'],
  ADD COLUMN IF NOT EXISTS default_labour_rate_per_hour numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS default_message_channel text NOT NULL DEFAULT 'whatsapp';

-- Public status page needs phone/address/hours readable by anon — extend
-- get_garage_job_status_public() rather than granting broad organisation
-- access.
-- Changing the OUT parameter row shape requires a drop, not just CREATE OR REPLACE.
DROP FUNCTION IF EXISTS public.get_garage_job_status_public(uuid);
CREATE FUNCTION public.get_garage_job_status_public(_job_id uuid)
RETURNS TABLE (
  job_number int,
  job_year int,
  status text,
...
BEGIN
  ...
  RETURN QUERY SELECT
    ...
    o.name, o.phone, o.address, o.business_hours,
  ...
  FROM public.organisations o WHERE o.id = v_job.organisation_id;
END;
$$;
