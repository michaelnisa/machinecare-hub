...
export function WorkOrderPtwCard({ wo, onSaved }: Props) {
  ...
  // Safety review team: owner/manager (supervisor), or anyone in the Safety
  // department — matches can_review_safety() in the DB. Deliberately drops
  // the old "no department set" loophole, which let any user without a
  // department review permits.
  ...
  const request = async () => {
    if (!profile) return;
    setRequesting(true);
    const { error } = await (supabase as any).from("wo_safety_approvals").insert({
    ...
    toast.success("Permit to Work requested");
    load();
    onSaved();
  };
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="mb-3 flex items-center gap-2 text-sm font-medium">
        <ShieldAlert className="h-4 w-4 text-amber-600" /> Permit to Work
      </div>

      {!approval && (
        <div className="flex items-center justify-between gap-3">
          <p className="text-xs text-muted-foreground">No permit requested for this work order yet.</p>
      ...
      {approval?.status === "pending" && (
        <div className="space-y-3">
          <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-xs text-amber-800">
            Pending Safety approval — requested by {names[approval.requested_by ?? ""] ?? "—"}{" "}
            {formatDistanceToNow(new Date(approval.requested_at), { addSuffix: true })}. Work can't start until this is approved.
          </div>
          {canReview && (
