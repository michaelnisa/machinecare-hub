...
export default function LiveProduction() {
  ...
  const load = useCallback(async () => {
    ...
    const downtime = prod.reduce((s: number, r: any) => s + Number(r.downtime_minutes ?? 0), 0);
  ...
  useEffect(() => {
    if (!organisation?.id) return;
    const channel = supabase.channel(`${organisation.id}:live-production`);
    channel
    ...
    <div className="min-h-screen w-full bg-[#0a0e1a] bg-[radial-gradient(ellipse_at_top,rgba(56,189,248,0.15),transparent_50%),radial-gradient(ellipse_at_bottom,rgba(168,85,247,0.12),transparent_50%)] text-white">
      <div className="mx-auto max-w-[1800px] px-8 py-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold tracking-tight">{organisation?.name ?? "Production"}</h1>
            <p className="mt-1 text-white/50">Production department — live KPI dashboard</p>
          </div>
        ...
        <div className={`grid grid-cols-1 gap-5 ${kpis.costEnabled ? "md:grid-cols-4" : "md:grid-cols-3"}`}>
          <Tile
          ...
          {kpis.costEnabled && (
            <Tile
              icon={<AlertTriangle className="h-6 w-6" />}
              label="Cost Lost"
              value={formatTZS(kpis.costLost)}
              sub="Downtime + scrap, today"
              accent={kpis.costLost === 0 ? "text-emerald-400" : "text-rose-400"}
              big={false}
            />
          )}
        </div>
