...
export default function SafetyLiveTV() {
  ...
  return (
    <div className="min-h-screen bg-[#021810] text-slate-100 selection:bg-emerald-500 selection:text-white font-sans">
      {/* Top Broadcast Navigation & Header */}
      <header className="sticky top-0 z-50 border-b border-emerald-800/40 bg-[#021810]/95 backdrop-blur-md px-6 py-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            {organisation?.logo_url ? (
              <img
                ...
                className="h-12 w-12 rounded-2xl object-contain bg-white/10 p-1.5 border border-emerald-500/30 shadow-lg shadow-emerald-950/50"
              />
            ) : (
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-800 border border-emerald-500/30 shadow-lg shadow-emerald-950/50 text-white font-black text-xl">
          ...
          <div className="flex items-center gap-6">
            <div className="hidden lg:flex items-center gap-3 rounded-xl border border-emerald-700/40 bg-[#04261b]/80 px-4 py-2">
              <Clock className="h-4 w-4 text-emerald-400" />
              <div className="text-right">
                <div className="font-mono text-lg font-bold text-white leading-tight">{timeStr}</div>
                <div className="text-[11px] text-emerald-200/60">{dateStr}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-emerald-700/40 bg-[#04261b]/80 px-3.5 py-2 text-xs font-bold text-emerald-100 hover:bg-emerald-900/50 transition-colors"
              >
      ...
      <main className="p-6 md:p-8 space-y-6 max-w-[1920px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 relative overflow-hidden rounded-3xl border border-emerald-600/40 bg-gradient-to-br from-[#063826] via-[#04271a] to-[#021810] p-8 shadow-2xl backdrop-blur">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2 text-xs text-emerald-200/70 font-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
              <div className="md:col-span-8 space-y-2">
                <div className="text-emerald-100/90 text-sm font-semibold uppercase tracking-wider">
                  Continuous Operation Without Lost Time Injury (LTI)
                </div>
                <div className="text-7xl sm:text-8xl md:text-9xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-emerald-200 via-teal-100 to-white">
                  {kpis.daysWithoutLti}
                </div>
                <div className="text-base text-emerald-300 font-medium flex items-center gap-2">
                  <CheckCircle2 className="h-5 w-5 text-emerald-400" />
                  All production lines and workshop cells operating incident-free today
                </div>
              </div>

              <div className="md:col-span-4 bg-[#02140d]/80 border border-emerald-700/40 rounded-2xl p-5 space-y-3">
                <div className="text-xs uppercase font-bold text-emerald-300/80 tracking-wider">Safety Compliance Target</div>
          ...
          <div className="lg:col-span-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
            <div className="rounded-2xl border border-emerald-700/40 bg-gradient-to-br from-[#052e1f] to-[#021810] p-6 shadow-xl relative overflow-hidden">
              ...
              <div className="mt-2 text-5xl font-black text-emerald-200">{kpis.activeApprovedPermits}</div>
            ...
            <div className="rounded-2xl border border-emerald-700/40 bg-gradient-to-br from-[#052e1f] to-[#021810] p-6 shadow-xl relative overflow-hidden">
              ...
              <div className="mt-2 text-5xl font-black text-teal-200">{kpis.activeContractors}</div>
              <div className="mt-2 text-xs text-emerald-200/70">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          <div className="rounded-2xl border border-emerald-800/40 bg-[#032116]/80 p-5 backdrop-blur">
            <div className="flex items-center justify-between text-emerald-300/80 text-xs font-bold uppercase">
              <span>Contractors</span>
              <Users className="h-4 w-4 text-emerald-400" />
            </div>
            <div className="mt-2 text-3xl font-extrabold text-white">{kpis.activeContractors}</div>
            <div className="text-[11px] text-emerald-300 font-medium mt-1">Verified on site</div>
          </div>

          <div className="rounded-2xl border border-emerald-800/40 bg-[#032116]/80 p-5 backdrop-blur">
            <div className="flex items-center justify-between text-amber-300/80 text-xs font-bold uppercase">
              <span>Pending RAMS</span>
              <FileWarning className="h-4 w-4 text-amber-400" />
            </div>
            <div className={`mt-2 text-3xl font-extrabold ${kpis.pendingRiskAssessments > 0 ? "text-amber-400 animate-pulse" : "text-emerald-100"}`}>
              {kpis.pendingRiskAssessments}
            </div>
            <div className="text-[11px] text-amber-300 font-medium mt-1">Awaiting Safety review</div>
          </div>
          ...
          <div className="rounded-2xl border border-emerald-800/40 bg-[#032116]/80 p-5 backdrop-blur">
            ...
            <div className="text-[11px] text-emerald-200/70 font-medium mt-1">Expiring in 30 days</div>
          </div>
          ...
          <div className="rounded-2xl border border-emerald-800/40 bg-[#032116]/80 p-5 backdrop-blur">
            ...
            <div className="text-[11px] text-emerald-300 font-medium mt-1">On duty this shift</div>
          </div>

          <div className="rounded-2xl border border-emerald-800/40 bg-[#032116]/80 p-5 backdrop-blur">
            <div className="flex items-center justify-between text-orange-300/80 text-xs font-bold uppercase">
        ...
        <div className="rounded-3xl border border-emerald-800/40 bg-[#032116]/80 p-6 backdrop-blur space-y-4">
          <div className="flex items-center gap-2 text-emerald-400 font-bold uppercase tracking-wider text-sm">
            <PhoneCall className="h-4 w-4" /> Emergency Duty Roster & Safety Protocol
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="rounded-xl border border-emerald-700/40 bg-[#021810]/70 p-4 space-y-1">
              <div className="text-[10px] uppercase font-bold text-emerald-300/80">Chief Safety Officer</div>
            ...
            <div className="rounded-xl border border-emerald-700/40 bg-[#021810]/70 p-4 space-y-1">
              <div className="text-[10px] uppercase font-bold text-emerald-300/80">Site Emergency Assembly</div>
              <div className="text-sm font-bold text-white">Muster Point #1 (Main Gate)</div>
              <div className="text-xs text-emerald-400 font-mono">Radio Channel: VHF 04</div>
            </div>

            <div className="rounded-xl border border-emerald-700/40 bg-[#021810]/70 p-4 space-y-1">
              <div className="text-[10px] uppercase font-bold text-emerald-300/80">Site Medical Station</div>
              <div className="text-sm font-bold text-white">Main Clinic Bay A</div>
              <div className="text-xs text-emerald-400 font-mono">Nurse On Duty • Ext 204</div>
            </div>

            <div className="rounded-xl border border-emerald-700/40 bg-[#021810]/70 p-4 space-y-1">
              <div className="text-[10px] uppercase font-bold text-emerald-300/80">Hazard Response Team</div>
              <div className="text-sm font-bold text-white">Spill & Fire Response Post 2</div>
              <div className="text-xs text-emerald-400 font-mono">Standby • VHF 09</div>
            </div>
          </div>
        </div>

        {/* BOTTOM REAL-TIME ROLLING TICKER */}
        <div className="rounded-2xl border border-emerald-800/40 bg-[#02140d] px-6 py-3.5 overflow-hidden flex items-center gap-4">
          ...
          <div className="overflow-hidden whitespace-nowrap w-full">
            <div className="inline-block animate-marquee text-xs font-medium text-emerald-100/90 space-x-12">
