...
export default function InventoryHistory() {
  ...
  const load = async () => {
    ...
    let query = (supabase as any)
      ...
      .select(
        "*, inventory_items(name, part_number, unit), stock_locations(name), machines(name), work_orders(wo_number), profiles(full_name)",
      )
  ...
  useEffect(() => {
    load();
  }, [profile, itemFilter, typeFilter, from, to]);
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Every stock movement — immutable, with who, when, why, and what it's
          linked to.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="No stock movements match this filter."
        />
      ) : (
              ...
              <tbody>
                {txns.map((t) => (
                  <tr key={t.id} className="border-t border-border">
                    ...
                    <td
                      className={`px-5 py-3 font-medium ${Number(t.quantity) > 0 ? "text-emerald-600" : "text-red-600"}`}
                    >
                      {Number(t.quantity) > 0 ? "+" : ""}
                      {formatNumber(t.quantity)} {t.inventory_items?.unit}
                    </td>
                    <td className="px-5 py-3 text-muted-foreground">
                      {t.stock_locations?.name ?? "—"}
                    </td>
                    <td className="px-5 py-3 text-muted-foreground">
                      {t.machines?.name}
                      ...
                      {t.work_orders ? (
                        <Link
                          to={`/work-orders/${t.work_order_id}`}
                          className="text-primary hover:underline"
                        >
                          WO-{t.work_orders.wo_number}
                        </Link>
                      ) : null}
