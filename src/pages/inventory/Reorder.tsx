...
export default function Reorder() {
  ...
  const [usage, setUsage] = useState<
    Record<string, { last30: number; prev30: number }>
  >({});
  ...
  const load = async () => {
    ...
    const [
      ...
      supabase
        ...
        .select(
          "id, name, part_number, unit, unit_cost, reorder_level, reorder_quantity, safety_stock, lead_time_days, avg_monthly_consumption, criticality, status",
        )
        .eq("status", "active"),
      (supabase as any)
        .from("stock_balances")
      ...
      (supabase as any)
        ...
        .select("item_id, quantity, transaction_type, created_at")
        .in("transaction_type", ["issue", "consumption", "scrap", "damage"])
    ...
    (txns ?? []).forEach((t: any) => {
      ...
      if (new Date(t.created_at) >= day30) e.last30 += qtyAbs;
      else e.prev30 += qtyAbs;
      u[t.item_id] = e;
    });
    setUsage(u);
    setLoading(false);
  ...
  const recommend = (item: any) => {
    ...
    if (item.reorder_quantity && Number(item.reorder_quantity) > 0)
      return Number(item.reorder_quantity);
    const monthly = Number(
      item.avg_monthly_consumption ?? usage[item.id]?.last30 ?? 0,
    );
    const leadMonths = Number(item.lead_time_days ?? 30) / 30;
    const target = Number(item.safety_stock ?? 0) + monthly * leadMonths;
    return Math.max(
      1,
      Math.ceil(target - avail),
      Math.ceil(Number(item.reorder_level ?? 0) - avail),
    );
  };
  ...
  const trending = useMemo(() => {
    return items
      .map((i) => {
        ...
        const change =
          u.prev30 > 0
            ? ((u.last30 - u.prev30) / u.prev30) * 100
            : u.last30 > 0
              ? 100
              : 0;
      ...
      .filter(
        (r) =>
          r.last30 > 0 && (r.change >= 20 || (r.prev30 === 0 && r.last30 > 0)),
      )
      .sort((a, b) => b.change - a.change)
      .slice(0, 10);
  }, [items, usage]);

  const criticalOut = needsReorder.filter(
    (r) => r.item.criticality === "critical" && r.available <= 0,
  ).length;

  useEffect(() => {
    const q: Record<string, string> = {};
    needsReorder.forEach((r) => {
      q[r.item.id] = String(r.recommended);
    });
    setQty((prev) => ({ ...q, ...prev }));
  }, [needsReorder.length]);
  ...
  const createRequests = async () => {
    ...
    toast.success(
      `Purchase request created for ${rows.length} item${rows.length === 1 ? "" : "s"}`,
    );
    ...
    <div className="space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          What's at or below its reorder point, what's trending, and what to do
          about it.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div
            className={`mt-1 text-2xl font-semibold ${needsReorder.length > 0 ? "text-amber-600" : ""}`}
          >
            {needsReorder.length}
          </div>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="text-xs uppercase tracking-wide text-muted-foreground">
            Critical spares out of stock
          </div>
          <div
            className={`mt-1 text-2xl font-semibold ${criticalOut > 0 ? "text-red-600" : ""}`}
          >
            {criticalOut}
          </div>
        </div>
                  ...
                  {needsReorder.map((r) => (
                    <tr key={r.item.id} className="border-t border-border">
                      <td className="px-4 py-3">
                        <input
                          type="checkbox"
                          checked={!!selected[r.item.id]}
                          onChange={(e) =>
                            setSelected((s) => ({
                              ...s,
                              [r.item.id]: e.target.checked,
                            }))
                          }
                        />
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {formatNumber(r.available)} {r.item.unit}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">
                        {formatNumber(r.item.reorder_level)} {r.item.unit}
                      </td>
                      <td className="px-4 py-3">
                        <Input
                          type="number"
                          min={1}
                          step="any"
                          value={qty[r.item.id] ?? String(r.recommended)}
                          onChange={(e) =>
                            setQty((q) => ({
                              ...q,
                              [r.item.id]: e.target.value,
                            }))
                          }
                          className="h-8 w-24"
                        />
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs ${r.available <= 0 ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                        >
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-medium">
          Demand trend (last 30 days vs prior 30)
        </h2>
        <p className="text-xs text-muted-foreground">
          A recommended signal based on actual consumption, not a guaranteed
          prediction — review before acting.
        </p>
        {trending.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No item's usage has risen noticeably in the last 30 days.
          </p>
        ) : (
          <div className="overflow-hidden rounded-xl border border-border bg-card">
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {trending.map((r) => (
                    <tr key={r.item.id} className="border-t border-border">
                      ...
                      <td className="px-4 py-3 text-muted-foreground">
                        {formatNumber(r.prev30)} {r.item.unit}
                      </td>
                      <td className="px-4 py-3">
                        {formatNumber(r.last30)} {r.item.unit}
                      </td>
                      <td className="px-4 py-3">
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="max-w-md">
            ...
            {selectedRows.map((r) => (
              ...
              >
                ...
                <span className="text-muted-foreground">
                  {qty[r.item.id] ?? r.recommended} {r.item.unit}
                </span>
              </div>
            ))}
          </div>
          <DialogFooter>
            ...
            <Button onClick={createRequests} disabled={saving}>
