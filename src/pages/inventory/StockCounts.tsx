  ...
  const load = async () => {
    ...
    const [{ data: c, error: e1 }, { data: l, error: e2 }] = await Promise.all([
      (supabase as any)
        .from("stock_counts")
        .select(
          "*, stock_locations(name), stock_count_items(id, variance, expected_quantity, physical_quantity, inventory_items(unit_cost))",
        )
    ...
    () =>
      counts.map((c) => {
        ...
        const counted = lines.filter(
          (l: any) => l.physical_quantity !== null,
        ).length;
        const varianceValue = lines.reduce(
          (s: number, l: any) =>
            s +
            Math.abs(Number(l.variance ?? 0)) *
              Number(l.inventory_items?.unit_cost ?? 0),
          0,
        );
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Reconcile physical stock against the system, with reasons and
            variance-value approval.
          </p>
        </div>
      ...
      {withStats.length === 0 ? (
        <EmptyState
          ...
          description="Start a cycle count, a location count, or a full stocktake."
        />
      ) : (
                ...
                {withStats.map((c) => (
                  ...
                  >
                    <td className="px-5 py-3 font-medium">
                      {TYPE_LABEL[c.count_type] ?? c.count_type}
                    </td>
                    <td className="px-5 py-3 text-muted-foreground">
                      {c.stock_locations?.name ?? c.category ?? "All"}
                    </td>
                    <td className="px-5 py-3">
                      {c.counted}/{c.lineCount} counted
                    </td>
                    <td className="px-5 py-3">
                      {c.varianceValue > 0 ? formatMoney(c.varianceValue) : "—"}
                    </td>
                    <td className="px-5 py-3">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs capitalize ${STATUS_CLASS[c.status]}`}
                      >
                        {c.status}
                      </span>
                    </td>
...
function NewCountDialog({
  ...
  useEffect(() => {
    if (open) setForm({ count_type: "cycle", location_id: "", category: "" });
  }, [open]);

  const submit = async () => {
    if (form.count_type === "location" && !form.location_id)
      return toast.error("Select a location");
    if (form.count_type === "category" && !form.category.trim())
    ...
    if (form.count_type !== "cycle") {
      let query = (supabase as any)
        .from("stock_balances")
        .select(
          "item_id, location_id, physical_stock, inventory_items!inner(status, category, criticality)",
        )
        .eq("inventory_items.status", "active");
      if (form.count_type === "location")
        query = query.eq("location_id", form.location_id);
      if (form.count_type === "category")
        query = query.eq("inventory_items.category", form.category.trim());
      if (form.count_type === "critical_spares")
        query = query.eq("inventory_items.criticality", "critical");
      const { data: balances, error: balErr } = await query;
      if (balErr) {
        setSaving(false);
        return toast.error(balErr.message);
      }
      if (balances?.length) {
        const rows = balances.map((b: any) => ({
          ...
          expected_quantity: b.physical_stock,
        }));
        ...
        if (rowsErr) {
          setSaving(false);
          return toast.error(rowsErr.message);
        }
      }
    }
...
function CountDetailDialog({ count, onClose, canManage, onSaved }: any) {
  ...
  const load = async () => {
    ...
    const { data, error } = await (supabase as any)
      .from("stock_count_items")
      .select(
        "*, inventory_items(name, part_number, unit, unit_cost), stock_locations(name)",
      )
  ...
  const applyCount = async () => {
    setBusy(true);
    const { error } = await (supabase as any).rpc("apply_stock_count", {
      _count_id: count.id,
    });
    setBusy(false);
    if (error) return toast.error(error.message);
  ...
  return (
    <Dialog open={!!count} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {TYPE_LABEL[count.count_type] ?? count.count_type} —{" "}
            {count.stock_locations?.name ?? count.category ?? "All items"}
          </DialogTitle>
        </DialogHeader>
        ...
        {loadingLines ? (
          ...
          <EmptyState
            icon={<ClipboardCheck className="h-5 w-5" />}
            title="No items yet"
            description="Add items to count."
          />
        ) : (
                  ...
                  {lines.map((l) => (
                    <tr key={l.id} className="border-t border-border">
                      ...
                      <td className="px-4 py-2 text-muted-foreground">
                        {l.stock_locations?.name}
                      </td>
                      <td className="px-4 py-2">
                        {formatNumber(l.expected_quantity)}
                      </td>
                      <td className="px-4 py-2">
                        {isDraft ? (
                          <Input
                            ...
                            onBlur={(e) => {
                              ...
                              if (v !== l.physical_quantity)
                                updateLine(l.id, { physical_quantity: v });
                            }}
                          />
                        ) : (
                      ...
                      <td
                        className={`px-4 py-2 font-medium ${Number(l.variance) > 0 ? "text-emerald-600" : Number(l.variance) < 0 ? "text-red-600" : ""}`}
                      >
                      ...
                      <td className="px-4 py-2">
                        {isDraft ? (
                          <Input
                            ...
                            className="h-8 w-40"
                            placeholder={
                              Number(l.variance) !== 0 ? "Required" : "Optional"
                            }
                            onBlur={(e) => {
                              if (e.target.value !== (l.reason ?? ""))
                                updateLine(l.id, {
        ...
        {count.status === "submitted" && (
          <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-xs text-amber-800">
            ...
            {!canManage &&
              "This may need a manager to apply, depending on the org's approval threshold."}
          </div>
        )}
        ...
        <DialogFooter>
          ...
          {count.status === "submitted" && (
            <Button onClick={applyCount} disabled={busy}>
              {busy ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
                "Apply to stock"
...
function AddLineDialog({ open, setOpen, countId, onSaved }: any) {
  ...
  const submit = async () => {
    if (!form.item_id || !form.location_id)
      return toast.error("Select an item and location");
    setSaving(true);
    ...
    const { error } = await (supabase as any).from("stock_count_items").insert({
      ...
      expected_quantity: bal?.physical_stock ?? 0,
    });
    setSaving(false);
    if (error)
      return toast.error(
        error.code === "23505" ? "Already on this count" : error.message,
      );
