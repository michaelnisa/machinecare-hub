...
import { invoiceTotals } from "@/lib/garage-money";
...
export default function GarageInvoicePrint() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: invoice } = await (supabase as any)
        ...
        .select("*, garage_invoice_items(*), garage_jobs(*, garage_customers(name, phone, email, address), garage_vehicles(make, model, registration_number, mileage))")
        .eq("id", id)
        .maybeSingle();
      if (!invoice) { setLoading(false); return; }
      const [{ data: org }, { data: payments }] = await Promise.all([
        supabase.from("organisations").select("id,name,phone,address,invoice_footer_note").eq("id", invoice.organisation_id).maybeSingle(),
        (supabase as any).from("garage_payments").select("*").eq("invoice_id", id).order("paid_at", { ascending: true }),
      ]);
      setData({ invoice, job: invoice.garage_jobs, org, payments: payments ?? [] });
      setLoading(false);
    })();
  }, [id]);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`INV-${data.invoice.invoice_year}-${String(data.invoice.invoice_number).padStart(4, "0")}.pdf`);
    } catch (e: any) {
  ...
  if (!data?.invoice) {
    return (
      <div className="p-8 text-center">
        ...
        <Link to="/garage/invoices" className="text-primary hover:underline">Back to invoices</Link>
      </div>
    );
  }

  const { invoice, job, org, payments } = data;
  const totals = invoiceTotals(invoice);
  const items = invoice.garage_invoice_items ?? [];
    ...
    <div className="min-h-screen bg-muted/40 print:bg-white">
      ...
      <div ref={sheetRef} className="mx-auto my-6 w-[210mm] min-h-[297mm] bg-white p-[18mm] text-[11pt] text-slate-900 shadow-lg print:my-0 print:w-auto print:min-h-0 print:p-[14mm] print:shadow-none">
        <div className="flex items-start justify-between border-b-2 border-teal-600 pb-4">
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-700">Invoice</div>
            <div className="mt-1 text-3xl font-bold tracking-tight">INV-{invoice.invoice_year}-{String(invoice.invoice_number).padStart(4, "0")}</div>
            <div className="mt-1 text-xs text-slate-500">Issued {formatDate(invoice.issued_at)} · Job {formatJobNumber(job)}</div>
          </div>
          <div className="text-right">
            ...
            <div className={`mt-1 inline-block rounded-md border px-2 py-0.5 text-[10px] font-bold tracking-wide ${
              status === "PAID" ? "border-emerald-300 text-emerald-700" : status === "PARTIALLY PAID" ? "border-amber-300 text-amber-700" : "border-red-300 text-red-700"
            }`}>{status}</div>
          </div>
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-4">
          <Box title="Customer">
            ...
            {job.garage_customers?.address && <div className="text-xs text-slate-600">{job.garage_customers.address}</div>}
          </Box>
          <Box title="Vehicle">
        ...
        <div className="mt-4 flex justify-end">
          <div className="w-64 space-y-1.5 text-sm">
            {Number(invoice.labour_cost) > 0 && <Row label="Labour" value={formatMoney(invoice.labour_cost)} />}
            {Number(invoice.other_cost) > 0 && <Row label="Other" value={formatMoney(invoice.other_cost)} />}
            {Number(invoice.discount) > 0 && <Row label="Discount" value={`-${formatMoney(invoice.discount)}`} />}
            {Number(invoice.tax_rate_percent) > 0 && <Row label={`Tax (${invoice.tax_rate_percent}%)`} value={formatMoney(totals.taxAmount)} />}
            ...
            <div className="flex justify-between font-semibold"><span>Outstanding</span><span>{formatMoney(outstanding)}</span></div>
          </div>
        </div>

        {payments.length > 0 && (
          <div className="mt-6">
            <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">Payment history</div>
            <ul className="mt-1 space-y-0.5 text-xs text-slate-600">
              {payments.map((p: any) => (
                <li key={p.id}>{formatDate(p.paid_at)} — {p.type === "refund" ? "Refund " : ""}{formatMoney(p.amount)} via {p.method.replace(/_/g, " ")}{p.reference ? ` (${p.reference})` : ""}</li>
              ))}
            </ul>
          </div>
        )}
        ...
        <div className="mt-10 grid grid-cols-2 gap-8 text-xs text-slate-500">
          <div>
            ...
            <p className="mt-1">Issued by</p>
          </div>
          <div>
            <div className="h-10 border-b border-slate-400" />
            <p className="mt-1">Received by (signature)</p>
          </div>
        </div>

        <p className="mt-8 text-center text-[10px] text-slate-400">{org?.invoice_footer_note || "Thank you for your business."}</p>
      </div>
    </div>
  );
}
